...
public class MicHandle : MonoBehaviour {
	...
	void Start () {
		...
		StartCoroutine(StartMicListener());
	...
	private IEnumerator StartMicListener() {
	...
	void OnGUI(){
		GUI.TextField(new Rect (0,30,100,30), System.String.Format("Pitch: {0:F2}", this.pitchValue));
		GUI.TextField(new Rect (0,60,100,30), System.String.Format("RMS: {0:F2}", this.rmsValue));
		GUI.TextField(new Rect (0,90,100,30), System.String.Format("DB: {0:F2}", this.dbValue));
	}
}
